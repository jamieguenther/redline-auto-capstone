...
request.onload = function () {
  ...
  if (request.status >= 200 && request.status < 400) {
    data.forEach(vehicle => {
      if (("JUL" == vehicle.soldMonth || "AUG" == vehicle.soldMonth || "SEP" == vehicle.soldMonth ||
        "Jul" == vehicle.soldMonth || "Aug" == vehicle.soldMonth || "Sep" == vehicle.soldMonth) && year == vehicle.soldYear) {
        makeList(vehicle);
      }
    });
  }
