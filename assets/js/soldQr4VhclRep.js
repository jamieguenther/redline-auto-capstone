...
request.onload = function () {
  ...
  if (request.status >= 200 && request.status < 400) {
    data.forEach(vehicle => {
      if (("OCT" == vehicle.soldMonth || "NOV" == vehicle.soldMonth || "DEC" == vehicle.soldMonth ||
        "Oct" == vehicle.soldMonth || "Nov" == vehicle.soldMonth || "Dec" == vehicle.soldMonth) && year == vehicle.soldYear) {
        makeList(vehicle);
      }
    });
  }
